import { useEffect, useState } from "react"; // แสดง Report, Place และ Reporter แบบละเอียด | Admin เลือกปฏิเสธ Report หรือดำเนินการลบ Place
...
export default function AdminReportDetailPage() { // ประกาศฟังก์ชันนี้และกำหนดขอบเขตงานตามชื่อของฟังก์ชัน
  const { id } = useParams(); // id จาก URL ใช้โหลด Report พร้อม Place และ Reporter
  const navigate = useNavigate(); // อ่านค่าจาก React Hook ที่ Component ต้องใช้ในรอบ render นี้
  const [report, setReport] = useState(null); // สร้าง State และฟังก์ชันเปลี่ยนค่าเพื่อให้ React render ใหม่เมื่อข้อมูลเปลี่ยน
  useEffect(() => { // โหลดรายละเอียดครั้งแรกหรือเมื่อ id ใน URL เปลี่ยน
    api.get(`/reports/${id}`)
      .then((response) => setReport(response.data.report))
      .catch((error) => console.error(getErrorMessage(error)));
  }, [id]);
  if (!report) return <PageLoader />; // ยังไม่มี Response ให้แสดง Loading

  async function update(status) { // สร้างฟังก์ชัน async เพื่อให้ใช้ await กับงาน API หรือ Database ได้
    try { // ปุ่มปฏิเสธส่ง rejected; Backend จะลบรูปหลักฐานชั่วคราว
      await api.patch(`/reports/${id}/status`, { status }); // รอ Promise นี้ทำงานเสร็จก่อนใช้ผลลัพธ์หรือทำบรรทัดถัดไป
      navigate("/admin/reports"); // เปลี่ยน URL และนำผู้ใช้ไปยังหน้าที่กำหนด
    } catch (error) {
      console.error(getErrorMessage(error));
    }
  }

  async function deleteReportedPlace() { // สร้างฟังก์ชัน async เพื่อให้ใช้ await กับงาน API หรือ Database ได้
    const confirmed = window.confirm( // ใช้เมื่อหลักฐานยืนยันว่า Place ไม่มีจริงและต้องลบ Marker
      "ยืนยันลบสถานที่นี้? Marker และรายงานทั้งหมดของสถานที่จะถูกลบด้วย"
    );

    if (!confirmed) return; // ตรวจเงื่อนไขก่อนอนุญาตให้โค้ดภายในทำงาน

    try { // Backend ตั้ง onDelete: Cascade จึงลบ Reports ของ Place นี้ตามไปด้วย
      await api.delete(`/places/${report.place.id}`); // รอ Promise นี้ทำงานเสร็จก่อนใช้ผลลัพธ์หรือทำบรรทัดถัดไป
      navigate("/admin/reports"); // เปลี่ยน URL และนำผู้ใช้ไปยังหน้าที่กำหนด
    } catch (error) {
      console.error(getErrorMessage(error));
    }
