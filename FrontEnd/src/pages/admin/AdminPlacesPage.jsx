import { Trash2 } from "lucide-react"; // Admin ดู Place ทุกสถานะและลบสถานที่ | API หน้านี้ต้องผ่าน authMiddleware และ adminMiddleware
...
import { Chip, EmptyState } from "../../components/ui.jsx"; // นำ Dependency หรือ Module ที่บรรทัดถัดไปต้องใช้เข้ามาในไฟล์
import api, { getErrorMessage } from "../../services/api.js"; // นำ Dependency หรือ Module ที่บรรทัดถัดไปต้องใช้เข้ามาในไฟล์

export default function AdminPlacesPage() { // ประกาศฟังก์ชันนี้และกำหนดขอบเขตงานตามชื่อของฟังก์ชัน
  const [places, setPlaces] = useState([]); // เก็บข้อมูลทั้งหมดจาก Admin API และค่าที่ใช้ Filter บนหน้าจอ
  const [type, setType] = useState(""); // สร้าง State และฟังก์ชันเปลี่ยนค่าเพื่อให้ React render ใหม่เมื่อข้อมูลเปลี่ยน
  const [search, setSearch] = useState(""); // สร้าง State และฟังก์ชันเปลี่ยนค่าเพื่อให้ React render ใหม่เมื่อข้อมูลเปลี่ยน

  async function loadPlaces() { // สร้างฟังก์ชัน async เพื่อให้ใช้ await กับงาน API หรือ Database ได้
    try { // status=all ใช้ได้เฉพาะ Admin และคืน Place ทุกสถานะ
      const response = await api.get("/places", { params: { status: "all" } }); // รอ Promise นี้ทำงานเสร็จก่อนใช้ผลลัพธ์หรือทำบรรทัดถัดไป
  ...
  useEffect(() => { loadPlaces(); }, []); // โหลดรายการครั้งเดียวเมื่อเปิดหน้า

  async function remove(id) { // สร้างฟังก์ชัน async เพื่อให้ใช้ await กับงาน API หรือ Database ได้
    if (!window.confirm("ยืนยันการลบสถานที่นี้?")) return; // ยืนยันก่อนลบ เพราะ Backend จะลบรูปและ Reports ที่เกี่ยวข้องด้วย
    try { // เริ่มดักงานที่อาจเกิด Error เพื่อจัดการผลลัพธ์อย่างควบคุม
      await api.delete(`/places/${id}`); // รอ Promise นี้ทำงานเสร็จก่อนใช้ผลลัพธ์หรือทำบรรทัดถัดไป
      loadPlaces();
    } catch (error) {
      console.error(getErrorMessage(error));
    }
  ...
  const filtered = places.filter((place) => (!type || place.type === type) && place.name.toLowerCase().includes(search.toLowerCase())); // Filter นี้ทำใน Browser เพราะโหลด Place ทั้งหมดมาแล้ว
